    ...
    if (!in_background) {
        ...
        for (int i = 0; i < cmd_count; i++) {
            ...
            if (WIFEXITED(cmd_status)) {
                ...
                debug_log("[Parent] Command %d (pid %d) exited with status %d", 
                          i, pids[i], status);
            } else {
                debug_log("[Parent] Command %d (pid %d) terminated abnormally", i, pids[i]);
            }
        }
    } else {
        ...
        for (int i = 0; i < cmd_count; i++) {
            for (int j = 0; cmds[i][j] != NULL; j++) {
                if (command_str[0] != '\0') strcat(command_str, " ");
